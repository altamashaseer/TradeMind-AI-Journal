      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredTrades.map(trade => (
          <div 
            ...
            className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden hover:border-indigo-500/50 hover:shadow-lg hover:shadow-indigo-500/10 transition-all cursor-pointer group"
          >
            {trade.screenshotUrl ? (
                <div className="h-40 w-full overflow-hidden bg-slate-900 relative">
                    <img src={trade.screenshotUrl} alt="Chart" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                ...
                <div className="h-40 w-full bg-slate-800 border-b border-slate-700 flex items-center justify-center text-slate-600">
            ...
            <div className="p-5">
                <div className="flex justify-between items-start mb-2">
                    <div>
                        <h4 className="text-white font-bold text-lg">{trade.instrument}</h4>
                ...
                <div className="flex gap-2 text-xs text-slate-400 mb-3">
                    <span className={`px-1.5 py-0.5 rounded bg-slate-700/50 border border-slate-700`}>{trade.direction}</span>
                    {trade.setup && <span className="px-1.5 py-0.5 rounded bg-slate-700/50 border border-slate-700">{trade.setup}</span>}
                </div>
